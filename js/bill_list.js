...
class billList {
     ...
     * @param {string} tagID 传入父元素ID
     * 
     *
     */
    constructor(dataList, tagID) {
        this.dataList = dataList;
        this.tagID = tagID + "1"; //需改为随机字符
    }

    create(listContainer, elementType) {
        ...
        if (this.dataList != null) {
            for (var i = 0; i < this.dataList.length; i++) {
                ...
                cardtemplet.create(listContainer);

            };
        } else {
            listContainer.html('无');
        }
    }
}
...
class optionInput {
    ...
    create(faContainer) {
        var input_name = faContainer.attr("name");
        var input = $('<input class="form-check-input  " type="checkbox">').attr({
            name: input_name,
            id: this.tagID,
            value: this.value
        });
        var label = $('<label class=" btn btn-primary zebra btn-block "  data-toggle="button"></label>').attr({
            for: this.tagID,
        }).html(this.value);
        var span = $('<span class="col zebra-select-option " ></span>');

        span.append(label, input);

        faContainer.append(span);
    }
 ...
 * @param {array} bills bill数组
 * @param {class} elementType 子元素类id
 * @param {string} tagID jquery父元素id>生成或自定id，用于生成子元素id
 */
function updateElementListIn(faContainer, bills = global.dataJson.bills, elementType, tagID = generateId()) {
    var listContainer = new billList(bills, ("id" in faContainer[0]) ? faContainer[0].id : tagID);
    listContainer.create(faContainer, elementType);
}

function updateAccountList(faContainer) {
    if (global.dataJson.accounts == null) {
        var li = $("<li><a class='scroll-link'>" + "无账户" + "</a></li>");
